...
test('runtime modules and styles are cache-busted for existing PWA clients',async()=>{const [html,app,renderer]=await Promise.all([readFile(new URL('../index.html',import.meta.url),'utf8'),readFile(new URL('../assets/js/app.js',import.meta.url),'utf8'),readFile(new URL('../assets/js/renderers-v3.js',import.meta.url),'utf8')]);assert.match(html,/styles-v5\.css\?v=5\.0\.0/);assert.match(html,/app\.js\?v=5\.0\.0/);assert.doesNotMatch(app,/from ['"]\.\/[^'"]+\.js['"]/);assert.doesNotMatch(renderer,/from ['"]\.\/[^'"]+\.js['"]/)});
