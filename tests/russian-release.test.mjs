...
test('release version is consistent across runtime entry points',()=>{
  assert.equal(RELEASE_CONFIG.version,'4.1.0');
  ...
  assert.match(read('sw.js'),new RegExp(`markovlab-v${RELEASE_CONFIG.version.replaceAll('.','\\.')}-`));
});
...
test('calculator completeness matrix contains every stable calculator ID',()=>{
  ...
  assert.match(matrix,/Итого: \*\*85 калькуляторов из 85\*\*/);
});
