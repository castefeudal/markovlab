...
test('v1 state migrates to v3 without losing records',()=>{const x=migrateState({version:1,lang:'en',profile:{weight:80},favorites:['bmi'],history:[{calcId:'bmi'}],snapshots:[]});assert.equal(x.version,3);assert.equal(x.profile.weight,80);assert.deepEqual(x.recents,[])});
