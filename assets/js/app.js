...
document.addEventListener('keydown',e=>{if((e.metaKey||e.ctrlKey)&&e.key.toLowerCase()==='k'){e.preventDefault();palette.open?palette.close():openPalette()}if(e.key==='Escape')document.querySelectorAll('.data-popover.open').forEach(x=>x.classList.remove('open'))});
...
app.addEventListener('click',e=>{const action=e.target.closest('[data-action]')?.dataset.action;if(action==='settings-menu'||action==='data-menu'){const id=action==='settings-menu'?'settings-popover':'data-popover',other=action==='settings-menu'?'data-popover':'settings-popover',pop=document.querySelector(`#${id}`),btn=e.target.closest('button');document.querySelector(`#${other}`)?.classList.remove('open');pop?.classList.toggle('open');btn?.setAttribute('aria-expanded',String(pop?.classList.contains('open')));return}const lang=e.target.closest('[data-lang]')?.dataset.lang;if(lang&&['ru','en'].includes(lang)&&lang!==state.lang){state.lang=lang;persist();render();return}const theme=e.target.closest('[data-theme]')?.dataset.theme;if(theme&&['system','light','dark','midnight'].includes(theme)){state.theme=theme;persist();render();notify(`${t('theme',state.lang)}: ${t(theme,state.lang)}`)}});
