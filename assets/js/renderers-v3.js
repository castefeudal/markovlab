...
function proWorkbench(calc,lang){
  ...
  const scenario=numeric.length?`<div class="pro-sensitivity"><label>${bi('Параметр чувствительности','Sensitivity driver',lang)}<select data-pro-driver>${numeric.map(field=>`<option value="${esc(field.id)}">${esc(l(field.label,lang))}</option>`).join('')}</select></label><div class="scenario-buttons" role="radiogroup" aria-label="${bi('Изменение параметра','Driver adjustment',lang)}">${[-10,-5,5,10].map(value=>`<label class="btn"><input type="radio" name="pro-delta" value="${value}" data-pro-delta data-delta="${value}"><span>${value>0?'+':''}${value}%</span></label>`).join('')}</div><div class="pro-scenario-output" aria-live="polite"><p>${bi('Выберите параметр и изменение — MARKOVLAB сравнит базовый и альтернативный сценарии, не меняя форму.','Choose a driver and adjustment. MARKOVLAB will compare baseline and alternative scenarios without changing the form.',lang)}</p></div></div>`:`<p class="pro-no-scenario">${bi('У этой формулы нет числового параметра для честного сценарного анализа. Pro-режим раскрывает протокол и допущения, не выдумывая ложную точность.','This formula has no numeric driver for an honest sensitivity scenario. Pro mode exposes protocol and assumptions without inventing precision.',lang)}</p>`;
...
export function aboutPage(state){
  ...
  return legacy.aboutPage(state)
    .replace(/MARKOVLAB \/ 2\.0\.0/g,'MARKOVLAB / 5.1.0')
    ...
    .replace(/<p><strong>2\.0\.0<\/strong>/,`<p><strong>5.1.0</strong> — ${bi('повышенная читаемость, четыре визуально разные палитры плюс System, практические сценарии, авторский слой, editorial-фотосерия и Pro-анализ для всех 85 калькуляторов.','higher readability, four distinct visual palettes plus System, practical scenarios, author voice, an editorial photo series, and Pro analysis across all 85 calculators.',lang)}</p><p><strong>5.0.0</strong> — ${bi('search-first оболочка без постоянного sidebar, рабочая Home, компактная система результатов, новая измерительная айдентика и смысловая evidence-визуализация.','search-first shell without a permanent sidebar, useful Home workspace, compact result system, new measurement identity and meaningful evidence visual.',lang)}</p><p><strong>4.0.0</strong> — ${bi('natural-language поиск, персональная главная, RU/EN, четыре темы и release-контур.','natural-language search, personalized home, RU/EN, four themes and release quality gates.',lang)}</p><p><strong>3.1.0</strong> — ${bi('индивидуальные пояснения результата и проверяемая матрица 85 инструментов.','individual result guidance and a verifiable 85-tool matrix.',lang)}</p><p><strong>3.0.0</strong> — ${bi('единая визуальная система и семантически честные визуализации.','unified visual system and semantically honest visualizations.',lang)}</p><p><strong>2.0.0</strong>`);
}
