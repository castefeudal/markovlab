...
export function getProConfig(calc){
  ...
  const mode=override?.mode||(!numeric.length?'none':CATEGORY_DEFAULTS[calc.category]||'compare');
  if(mode==='none')return {mode:'none',inputs:[]};
  const allowed=(override?.inputs||numeric.map(field=>field.id)).map(id=>numeric.find(field=>field.id===id)).filter(Boolean);
